...
# ---- Zero-σ handling (your rule) + hard cap when σ>EPS ----
def cpk_with_zero_sigma_penalty(mu: float, sigma: float, spec: dict, lam: float) -> float:
    ...
    - If σ <= EPS:
        * If μ is OUTSIDE spec -> return -100 directly (no penalty).
        * If μ is INSIDE spec -> return 100 * exp(-λ * d_norm^2),
          where d_norm is distance from the spec center (normalized).
    """
    ...
    # σ ≈ 0: check inside vs outside first
    t = spec["type"].lower()
    if t == "two_sided":
        L, U = spec["L"], spec["U"]
        inside = (L <= mu <= U)
    ...
    if not inside:
        return -CPK_INF  # hard fail when σ≈0 and μ out of spec

    # μ is inside: apply centering penalty (or return +100 if λ=0)
    if lam <= 0.0:
        return CPK_INF
